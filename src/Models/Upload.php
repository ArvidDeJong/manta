...
use Darvis\Manta\Services\PdfToImage;
...
class Upload extends Model
{
    ...
    public function upload($file, $model, $model_id, array $options = [])
    {
        ...
        $disk = $options['disk'] ?? cms_config('manta')['upload']['disk'];

        $location = $options['location'] ?? 'uploads/' . env('THEME') . '/' . date('Y') . '/' . date('m') . '/' . date('d') . '/';
        ...
        if (in_array($upload->extension, ['jpg', 'jpeg', 'png'])) {
            foreach (cms_config('manta')['thumbnails'] as $size) {
                $upload->resize($size);
            }
        }
